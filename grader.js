...
var cheerioHtmlFile = function(htmlfile) {
	if (htmlfile.indexOf("http")==0){
		...
		var cheerio_results = cheerio.load(fs.readFileSync('tmp.txt'));
		fs.unlinkSync('tmp.txt');
		return cheerio_results;
	} else {
...
if(require.main == module) {
    ...
    if (program.url) {
    	...
    	fileToCheck = program.url;
    } else {
    	//console.log("Using file %s", program.file);
